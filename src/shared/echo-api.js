    ...
    try {
      ...
      if (response.ok) {
        ...
        try {
          ...
          console.warn(`[echo-api] Failed to parse JSON response: ${text.substring(0, 100)}`);
          return {};
        }
      }
...
const EchoAPI = {
   ...
   *
   * @param {string} query - Search query (set name or code)
   * @param {string} token - Authentication token
   * @returns {Promise<object[]>} Array of set objects
   */
  async searchSets(query, token) {
    return request("GET", "/data/sets", {
   ...
   * @returns {Promise<object>}
   */
  async editNote(noteId, noteText, token) {
    return request("POST", "/notes/edit/", {
      data: { id: noteId, note: noteText },
      token,
    });
  },
   ...
   *
   * @param {string|number} inventoryId - The inventory item's ID
   * @param {string} token
   * @returns {Promise<object>}
   */
  async getNotes(inventoryId, token) {
    return request("GET", "/notes/get/", {
  ...
  async getLists(token) {
    return request("GET", "/lists/all", { token });
  },

  /**
   * Fetch cards in a specific list.
   *
   * @param {string|number} listId
   * @param {string} token
   * @returns {Promise<object>}
   */
  async getList(listId, token) {
    return request("GET", "/lists/get", { params: { list: listId }, token });
  },

  /**
   * Search the user's inventory.
   *
   * @param {string} token
   * @param {object} [opts] - { start, limit, sort, direction }
   * @returns {Promise<object>}
   */
  async searchInventory(token, opts = {}) {
    return request("GET", "/inventory/search/", {
      params: { ...opts, limit: opts.limit || 100 },
      token,
    });
  },

  /**
   * Get user's full inventory (paginated).
   *
   * @param {string} token
   * @param {number} [start=0]
   ...
   * @returns {Promise<object>}
   */
  async getInventory(token, start = 0, limit = 100) {
    return request("GET", "/inventory/view/", {
      params: { start, limit },
      token,
    });
  },
